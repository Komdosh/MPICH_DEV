...
double round_trip_nb_overlap( int reps, int len, void *vctx)
{
    ...
    elapsed_time = 0;
    if(myproc==proc1){
	MPI_Recv(rbuffer,MsgSize,MPI_BYTE,proc2,0,MPI_COMM_WORLD,&status);
	t0=MPI_Wtime();
	for(i=0;i<reps;i++){
	    MPI_Irecv(rbuffer,MsgSize,MPI_BYTE,proc2,1,MPI_COMM_WORLD,&r[0]);
	    MPI_Isend(sbuffer,MsgSize,MPI_BYTE,proc2,1,MPI_COMM_WORLD,&r[1]);
	    OverlapComputation(len,ctx);
    ...
    if(myproc==proc2){
	MPI_Irecv(rbuffer,MsgSize,MPI_BYTE,proc1,1,MPI_COMM_WORLD,&r[0]);
	MPI_Isend(sbuffer,MsgSize,MPI_BYTE,proc1,0,MPI_COMM_WORLD,&r[1]);
	for(i=0;i<reps-1;i++){
	    OverlapComputation(len,ctx);
	    MPI_Waitall( 2, r, MPI_STATUSES_IGNORE );
	    MPI_Irecv(rbuffer,MsgSize,MPI_BYTE,proc1,1,MPI_COMM_WORLD,&r[0]);
	    MPI_Isend(sbuffer,MsgSize,MPI_BYTE,proc1,1,MPI_COMM_WORLD,&r[1]);
	}
	OverlapComputation(len,ctx);
	MPI_Waitall( 2, r, MPI_STATUSES_IGNORE );
...
double round_trip_b_overlap(int reps, int len, void *vctx)
{
	...
	for(i=0;i<reps;i++){
	    ...
	    OverlapComputation(len,ctx);
	    MPI_Recv(rbuffer,MsgSize,MPI_BYTE,proc2,1,MPI_COMM_WORLD,&status);
	}
	t1=MPI_Wtime();
	elapsed_time = t1 -t0;
    }
	...
	for(i=0;i<reps;i++){
	    OverlapComputation(len,ctx);
	    MPI_Recv(rbuffer,MsgSize,MPI_BYTE,proc1,1,MPI_COMM_WORLD,&status);
	    MPI_Send(sbuffer,MsgSize,MPI_BYTE,proc1,1,MPI_COMM_WORLD);
	}
    }
...
/* 
   Nonblocking round trip with overlap; this version subtracts the 
   time for the form with overlap from the time without overlap.

   Note: unlike the round_trip routines, the "length" in this routine 
   is the number of floating point operations.

   PROBLEMS:
   1) The automatic code for picking reps assumes that the time is absolute.
   With small (or even negative times), the adaptive code becomes confused,
   increasing reps to large values.

   2) Its not clear that the difference should be done within the test.
 */
double round_trip_nb_overlap_diff( int reps, int len, void *vctx)
{
    double elapsed_time;
    OverlapData *ctx = (OverlapData *)vctx;
    int  i,myproc,
	...
	/* Rerun, but without overlap */
	MPI_Recv(rbuffer,MsgSize,MPI_BYTE,proc2,2,MPI_COMM_WORLD,&status);
	t0=MPI_Wtime();
	for(i=0;i<reps;i++){
	    MPI_Irecv(rbuffer,MsgSize,MPI_BYTE,proc2,3,MPI_COMM_WORLD,&(r[0]));
	    MPI_Isend(sbuffer,MsgSize,MPI_BYTE,proc2,3,MPI_COMM_WORLD,&(r[1]));
	    MPI_Waitall( 2, r, MPI_STATUSES_IGNORE );
	    OverlapComputation(len,ctx);
	}
	t1=MPI_Wtime();
	elapsed_time = t1 -t0 - elapsed_time;
	printf( "[%d] time = %e, with %d\n", len, elapsed_time, reps ); fflush(stdout);
    }
	...
	/* Rerun, but without overlap */
	MPI_Irecv(rbuffer,MsgSize,MPI_BYTE,proc1,3,MPI_COMM_WORLD,&(r[0]));
	MPI_Isend(sbuffer,MsgSize,MPI_BYTE,proc1,2,MPI_COMM_WORLD,&(r[1]));
	for(i=0;i<reps-1;i++){
	    MPI_Waitall( 2, r, MPI_STATUSES_IGNORE );
	    OverlapComputation(len,ctx);
	    MPI_Irecv(rbuffer,MsgSize,MPI_BYTE,proc1,3,MPI_COMM_WORLD,&(r[0]));
	    MPI_Isend(sbuffer,MsgSize,MPI_BYTE,proc1,3,MPI_COMM_WORLD,&(r[1]));
	}
	MPI_Waitall( 2, r, MPI_STATUSES_IGNORE );
	OverlapComputation(len,ctx);
	MPI_Send(sbuffer,MsgSize,MPI_BYTE,proc1,3,MPI_COMM_WORLD);
    }
