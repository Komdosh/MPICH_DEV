...
/* This frees any storage associated with the value */
static int keyvalDelete( MPI_Comm comm, int key, void *value, void *state )
{
    if (value) {
	MPI_Comm *localcomm = (MPI_Comm *)value;
	/* This is the storage allocated to contain the MPI_Comm that was
...
void MPE_Seq_begin( MPI_Comm comm, int ng )
{
	...
	MPI_Keyval_create( MPI_NULL_COPY_FN, keyvalDelete, 
			   &MPE_Seq_keyval, NULL );
    }
    MPI_Attr_get( comm, MPE_Seq_keyval, (void *)&local_comm, &flag );

    if (!flag) {
	/* Allocate storage for the communicator; this allows the 
	   MPI_Comm to be an object of arbitrary size.  The free
	   function for this keyval will delete this storage. */
