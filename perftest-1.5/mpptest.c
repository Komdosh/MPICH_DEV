...
typedef struct _TwinResults {
    ...
    int    n_loop;          /* number of times the timing loop was
			       run and accepted */
    /* The following can be used to keep track of why mpptest adapted 
       the runs */
    int    nForWtick;       /* Number of times test failed wtick check, 
			       causing n_avg to be doubled */
    int    nForSmooth;      /* Number of times test was rerun to smooth
			       results */
...
void OutputDiagnostics( TwinResults *twin, FILE *fp );
...
int main( int argc, char *argv[] )
{
    ...
    /* Allow us to experiment with the cost of thread safety in the
       basic, micro benchmarks */
#ifdef HAVE_MPI_INIT_THREAD
    { int thread_level, provided;
    thread_level = MPI_THREAD_SINGLE;
    if (getenv("MPPTEST_THREAD_MULTIPLE")) thread_level = MPI_THREAD_MULTIPLE;
    if (SYArgHasName( &argc, argv, 1, "-threadmultiple"  )) {
        thread_level = MPI_THREAD_MULTIPLE;
    }
    MPI_Init_thread( &argc, &argv, thread_level, &provided );
    if (provided < thread_level) {
	fprintf( stderr, "Requested thread level was unavailable\n" );
    }
...
/* Get the output context */
    ...
    if (SYArgHasName( &argc, argv, 1, "-diagnostics" )) doDiagnostics = 1;
...
/* Pick the general test based on the presence of an -gop, -overlap, -bisect
   or no arg */
	...
	else if (SYArgHasName( &argc, argv, 1, "-diff" )) {
	    BasicCommTest = round_trip_nb_overlap_diff;
	    strcpy( protocol_name, "nonblocking diff" );
	}
	else {  /* Assume -async */
		    ...
		    double autorel, void *msgctx)
{
	...
	/* We want to have enough values for the initial tests to avoid 
	   missing interesting features.  We really should have an option
	   for this, but 32 fits the default tests (0 - 1k by 32 has 32 tests)
	*/
	SetResultsForStrided( first, last, (last-first)/32, twin );
    ...
    /* 
       Optionally generate some diagnostic information about the tests
     */
    if (doDiagnostics && myproc == 0) {
	printf( "\n#Test Diagnostics\n#MPI_Wtick value\t%e msec\n", 
		gwtick * 1000.0 );
	OutputDiagnostics( twin, stdout );
	fflush(stdout);
    }
...
"  Message test type:\n\
  ...
  -overlapmsgsize nn\n\
               ...
               If the size is negative, only perform the computation.\n\
...
/* Run a test for a single entry in the list. Return 1 if the test
   ...
   As an option when running on shared systems, check on the number
   of involuntary context switches; these indicate that the test times
   are distorted by contention with another job.  Unfortunately, though
   getrusage defines such a field, some Unix implementations (inluding 
   Linux) do not maintain this field.
 */
...
/* Output diagnostics about the run */
void OutputDiagnostics( TwinResults *twin, FILE *fp )
{
    TwinResults *twin_p = twin;

    fprintf( fp, "#len\ttick?\tsmooth?\ttests\tn_avg\tn_loop\t(tmax-tmin)/tmax%%\n" );
    while (twin_p) {
	double change;
	if (twin_p->n_loop < 1 || twin_p->ntests < 1) {
	    /* Skip any tests that we could not successfully run */
	...
	change = 0;
	if (twin_p->max_time > 0) 
	    change = (twin_p->max_time - twin_p->t) / twin_p->max_time;
	fprintf( fp, "%d\t%d\t%d\t%d\t%d\t%d\t%f\n", 
		 twin_p->len, twin_p->nForWtick, twin_p->nForSmooth,
		 twin_p->ntests, twin_p->n_avg, twin_p->n_loop, 
		 100 * change );
	twin_p = twin_p->next;
    }
}
...
/* 
   If the timing function treats size as units of something other than
   bytes, then the UpdateTimeRateScale function should be called.
   
   For example, if the unit of communication is a 4 byte unit, to make the
   rate come out in bytes/sec, call

   UpdateTimeRateScale( 1.0, 1.0/4.0 );

 */
void UpdateTimeRateScale( double tscale, double rscale )
{
    TimeScale = tscale;
    RateScale = rscale;
}
